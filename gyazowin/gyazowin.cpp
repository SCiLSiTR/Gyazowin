	...
	case WM_LBUTTONUP:
		{
			...
			*/
			...
			if (BitmapToClipboard(newBMP, NULL) == true) {
				...
				// PNG保存失敗...
				MessageBox(hWnd, _T("Unable to copy image"), szTitle, 
					MB_OK | MB_ICONERROR);
			}
...
////This was to copy the png image to clipboard
////does not work, instead use clipboardHBITMAP
...
//	Copys the bitmap to the clipboard
//http://www.codeproject.com/Questions/344732/copy-hbitmap-to-clipboard
BOOL BitmapToClipboard(HBITMAP hBM, HWND hWnd)
{
	if (!::OpenClipboard(hWnd))
		return false;
